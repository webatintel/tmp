...
//
// This file contains the Compute Shader to perform matrix A X matrix B
// 
...
{
	float a = IntelExt_WaveReadLaneAt(input.x, lane);
	float b = IntelExt_WaveReadLaneAt(input.y, lane);
	float c = IntelExt_WaveReadLaneAt(input.z, lane);
	float d = IntelExt_WaveReadLaneAt(input.w, lane);
